...
int mm_init(void) {
        ...
        if((seg_listp = mem_sbrk(LIST_NUM * DSIZE)) == (void *)-1)
                return -1;
        for(int i = 0; i < LIST_NUM; ++ i)
                PUT_SEGI(seg_listp, i, NULL);
...
static void *find_fit(size_t asize){
        ...
        for(int i = seglist_no; i < LIST_NUM; ++ i){
                for(bp = GET_SEGI(seg_listp, i); (bp != NULL) && \
...
static void *add_free_list(void *bp){
        ...
        if(GET_SEGI(seg_listp, seglist_no) == NULL){
                PUT_PREVP(bp, NULL);
                PUT_NEXTP(bp, NULL);
        }else if(GET_SEGI(seg_listp,seglist_no) != NULL){
                PUT_PREVP(bp, NULL);
                PUT_NEXTP(bp, GET_SEGI(seg_listp, seglist_no));
                PUT_PREVP(GET_SEGI(seg_listp, seglist_no), bp);
        }
        
        PUT_SEGI(seg_listp, seglist_no, bp);
        return bp;
}
...
static void delete_free_list(void *bp){
        ...
        if(bp == GET_SEGI(seg_listp, seglist_no))
                PUT_SEGI(seg_listp, seglist_no, next);
        if(prev != NULL)
                PUT_NEXTP(prev, next);
        if(next != NULL)
                PUT_PREVP(next, prev);
...
/*
 * Check the following elements for each block in heap_listp
 *   -Check each block's address alignment
 *   -Check heap boundaries
