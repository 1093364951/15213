...
/* Here are helper routines that I've written for myself */
int buildin(struct cmdline_tokens *tokens_ptr);
void Listjobs(struct cmdline_tokens *tokens_ptr);
void bgjob(struct cmdline_tokens *tokens_ptr);
void fgjob(struct cmdline_tokens *tokens_ptr);
...
/*
 * buildin - Excute the buildin command (quit, jobs, bg or fg)
 * 
 * If it is a build-in command, then just execute it immediately
 * Otherwise, fork a child process and run the job in the context 
 * of the child.
 *
 * Returns:
 *   1:                 if it is a build-in command
 *   0:                 if it isn't a build-in command
 */
int buildin(struct cmdline_tokens *tokens_ptr){
    switch(tokens_ptr->builtins){
        case BUILTIN_NONE:
            return 0;
        case BUILTIN_QUIT:
            exit(0);
...
/* Execute the built-in foreground command */
void fgjob(struct cmdline_tokens *tokens_ptr){
    struct job_t *job_ptr = NULL;
    int argc = tokens_ptr->argc;
    char **argv = tokens_ptr->argv;
    ...
    /* update the status of the process and send SIGCONT signal */
    job_ptr->state = FG;
    kill(job_ptr->pid, SIGCONT);
    ...
    while(fgpid(job_list) != 0 
        && getjobpid(job_list, job_ptr->pid)->state != ST){
        sigsuspend(&mask);
    }
}
...
void redirect_io(char *infile, char *outfile){
    ...
    FILE *fin, *fout;
    fin = fout = NULL;
    /* if input stream needs to be redirected */ 
    if(infile != NULL){
    ...
    /* if output stream needs to be redirected */
    if(outfile != NULL){
...
void sigint_handler(int sig) {
    ...
    if(pid != 0) {
        /* send SIGINT to the foreground job and check the return value */
        if(kill(-pid, SIGINT) != 0) {
            unix_error("Error: error when sending sigint\n");
        }
    }       
    return;
}
...
void sigtstp_handler(int sig) {
    ...
    pid_t pid = fgpid(job_list);
    if(pid != 0){
        /* send SIGTSTP to the foreground process and check the return value */
        if(kill(-pid, SIGTSTP) != 0) {
            unix_error("Error: error when sending sigtstp\n");
        }
    }
    return;
}
